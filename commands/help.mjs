...
async function help(msg, args) {
    ...
    if(args.length) {
        switch(args[0]) {
            ...
            case "ping":
                // tell how =ping works
                msg.channel.send(`\`${prefix}ping\` makes me respond with \`Pong!\`.`);
            break;
            case "kauser":
                // tell how =kauser works
                msg.channel.send(`\`${prefix}kauser [username or kaid]\` makes me respond with the corresponding Khan Academy user information.`);
            break;
            case "kaprogram":
                // tell how =kaprogram works
                msg.channel.send(`\`${prefix}kaprogram [program ID]\` makes me respond with the corresponding Khan Academy program information.`);
            break;
            case "define":
                // tell how =define works
                msg.channel.send(`\`${prefix}define [word]\` makes me give a definition or set of definitions for a word.`);
            break;
            case "pronounce":
                // tell how =pronounce works
                msg.channel.send(`\`${prefix}pronounce [word]\` makes me link a YouTube video of how to pronounce your word.`);
            break;
            case "translate":
            ...
            case "reactrole":
                ...
                msg.channel.send(`\`${prefix}reactrole [role name] [emoji]...\` makes me make a reaction role. Users can react with \`[emoji]\` on the message to make me give them \`[role name]\`.
            ...
            case "reactroleone":
                // tell how =reactrole works
                msg.channel.send(`\`${prefix}reactroleone [role name] [emoji]...\` makes me make a reaction role. Users can react with \`[emoji]\` on the message to make me give them \`[role name]\`.
            ...
            case "poll":
                // tell how =poll works
                msg.channel.send(`\`${prefix}poll [option] [emoji]...\` makes me hold a poll. Users can react with \`[emoji]\` on the message to make me give them \`[role name]\`, but they can only react once, so voting is fair. Options must be contained in quotation marks if spaces are present.
            ...
            case "diebot":
                ...
                msg.channel.send(`\`${prefix}diebot\` makes me delete my messages`);
            break;
            case "clear":
                // tell how =clear works
                msg.channel.send(`\`${prefix}clear [number]\*\` makes me delete \`[number]\` messages. If number is not specified, I will delete 99 messages. Older messages might not be deleteable`);
            break;
            case "clearall":
                // tell how =clearall works
                msg.channel.send(`\`${prefix}clearall\` makes me delete *every single message in the channel*. Use this command with caution!`);
            break;
            case "mute":
                // tell how =mute works
                msg.channel.send(`\`${prefix}mute [user] [time]\* [reason]\*\` makes me mute a user for \`[time]\` of format \`[number][unit of time]\` or a default of one week`);
            break;
            case "unmute":
                // tell how =unmute works
                msg.channel.send(`\`${prefix}unmute [user]\` make me unmute a muted user early`);
            break;
            case "addword":
                // tell how =unmute works
                msg.channel.send(`\`${prefix}addword [word]\` make me add a word to a list of censored words`);
            break;
            case "removeword":
                // tell how =unmute works
                msg.channel.send(`\`${prefix}removeword [word]\` make me remove a word from a list of censored words`);
            break;
            case "checkwords":
                // tell how =unmute works
                msg.channel.send(`\`${prefix}checkwords\` I'll send a list of censored words. Make sure to use this in a staff only channel!`);
            break;
            case "prefix":
                // tell how =prefix works
                msg.channel.send(`\`${prefix}prefix [new prefix]\*\` I'll state my prefix or set it to a new one!`);
            break;
            case "warn":
                // tell how =warn works
                msg.channel.send(`\`${prefix}warn [user] [reason]\` makes me store a warning for [user] about [reason] to review for various purposes`);
            break;
            case "infractions":
                // tell how =infractions works
                msg.channel.send(`\`${prefix}infractions [user]\` makes me list stored warnings/infractions of [user]`);
            break;
            case "gettext":
                // tell how =gettext works
                msg.channel.send(`\`${prefix}gettext\` makes me list the text from any PNG or JPEG files attached`);
            break;
            case "funnytext":
                // tell how =gettext works
                msg.channel.send(`\`${prefix}funnytext [filters]...\* [message]\` makes me post a message from you made a little more fun. Use \`uwu\`, \`owo\`, \`blarb\`, \`piglatin\`, \`shakespeare\`, \`lego\`, or \`reverse\` or you can use multiple! Find out what each one does and use several for your own combination`);
            break;
            case "invite":
                msg.channel.send(`\`${prefix}invite\` makes me give you my invite so you can add me to other servers`);
            break;
            case "github":
                msg.channel.send(`\`${prefix}invite\` makes me give you my GitHub repo link`);
            break;
            case "avatar":
                msg.channel.send(`\`${prefix}avatar [user]\*\` I'll send a larger picture of your avatar or the user mentioned in the command`);
            break;
            case "binary":
                msg.channel.send(`\`${prefix}binary [text]\*\` I'll convert to or from binary`);
            break;
        }
    } else {
        var embed = new MessageEmbed()
