...
export default {
    data: new SlashCommandBuilder()
        .setName("reactroleone")
        .setDescription("Create a post users can react to in order to get one role"),
    description: `reactroleone [role name] [emoji]...\` makes me make a reaction role with only one role per user. Users can react with \`[emoji]\` on the message to make me give them \`[role name]\`, but they can't get more than one.
